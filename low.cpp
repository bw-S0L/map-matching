...
inline void DJS(condidate u, condidate v, double& dis, double& F) {
...
inline void PDJC(condidate u, condidate v, double& F, double& dis) {
	int i = u.idfi, j = v.idfi;
	int l = u.idse, r = v.idse;
	F = 1;
	dis = 0;
	...
	for (int i = 1; i < TMP.size(); i++) {
		...
		for (int j = 0; j < TMP[i].size(); j++) {
			...
			for (int k = 0; k < TMP[i - 1].size(); k++) {
				(PDJC(TMP[i - 1][k], TMP[i][j], F[k], dis_r[k]));
